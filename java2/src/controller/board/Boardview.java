...
public class Boardview implements Initializable{
    ...
    // ** 댓글 테이블 메소드[메소드화 시킨 이유 : 여러번 테이블을 호출하기 위해]
    ...
    void delete(ActionEvent event) {
    	// 1. 경고 메시지 알림
    	Alert alert = new Alert(AlertType.CONFIRMATION); // ONFIRMATION : 확인/취소가 있는 버튼
    		alert.setHeaderText("해당 게시물을 삭제할까요?");
    	Optional<ButtonType> optional = alert.showAndWait(); // showAndWait() 메소드 반환타입 => 선택한 버튼
    		// Optional 클래스 : null을 객체로 저장하는 클래스
    	if(optional.get() == ButtonType.OK) { // 2. 확인 버튼을 눌렀을때
        	// 3. 삭제처리
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		Board board = controller.board.Board.board; // board 컨트롤내 테이블에서 선택된 객체 호출
		...
		// 만약에 작성자의 현재로그인된 id와 동일하지 않으면
		if(! board.getBwrite().equals(Login.member.getMid())) { // !: 부정
			btndelete.setVisible(false); // 버튼 숨기기
			btnupdate.setVisible(false); // false = 버튼숨기기 / true = 버튼 보이기
		}
		// 텍스트필드 컨트롤 수정 못하게 수정금지 
		txttitle.setEditable(false);
		txtcontent.setEditable(false);
