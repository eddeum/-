...
public class Day09_6_은행계좌프로그램 {
	...
	// ** main 밖에서 선언하는 이유
		...
		// static 변수는 프로그램 시작시 메모리할당 -> 프로그램 종료시 메모리초기화
		...
		while(true) { // 프로그램 무한루프 [종료조건 x]
			...
			if(선택==1) {
				...
				if(result) {
					...
					System.out.println("** 입금 실패(동일한 계좌가 없습니다.) **");
				} // else e
				...
				System.out.println("계좌번호 : "); 	String 계좌번호 = scanner.next();
				System.out.println("비밀번호 : "); 	String 비밀번호 = scanner.next();
				System.out.println("이체액 : ");		int 이체액 = scanner.nextInt();
				System.out.println("이체계좌 : "); 	String 이체계좌 = scanner.next();
				
				 int result = 컨트롤.이체(계좌번호, 비밀번호, 이체액, 이체계좌);
				...
				System.out.println("*** 검색한 계좌주님의 계좌목록 ***");
				int i = 1;
				for(은행 temp : 내계좌목록) {
					if(temp != null) System.out.println(i + "번 계좌번호 : " + temp.get계좌번호());
					i++;
				} // for e
