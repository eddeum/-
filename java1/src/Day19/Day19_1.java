...
public class Day19_1 {
	...
	public static void main(String[] args) {
		...
		// 컬렉션(수집=저장)프레임워크(미리만들어진클래스)
			...
			// 1. List 컬렉션
				// * 인덱스(저장순서)가 있음
				// 1. Arraylist 클래스 	: 싱글스레드 / 순차적 추가, 삭제 빠름
				// 2. Vector 클래스 		: 멀티스레드(동기화O)
				// 3. LinkedList 클래스	: 중간에 삽입/삭제 빠름
			// 2. Set 컬렉션
				// * 인덱스 X(저장순서가 없기떄문에 자동으로 객체가 중복불가능)
				// 1. HashSet 클래스
...
// 1. HashSet : 컬렉션프레임워크 클래스 // 2. <제네릭> : Set 클래스 안에 들어가는 자료형 // 3. set : 객체명(아무거나) // 4. new : 객체 생성시 메모리 할당 // 5. HashSet<생략가능>() : 생성자
		...
		// 4. Iterator : 저장된 객체를 한번씩 가져오는 반복자 리턴(순회)
			// * set 컬렉션에서 사용되는 이유 : 인덱스가 없기때문에 객체를 호출하는 방법x
