...
// jquery : js 프레임워크
	// $ : 제이쿼리(라이브러리 설치 : header파일 body태그에 cdn 추가)
...
$(function(){ // 문서 열리면 해당 코드가 실행
	...
	// 비밀번호 체크
	$("#mpassword").keyup(function(){ // 비밀번호를 입력할때마다
		...
		if(passwordj.test(mpassword) ){ // 정규표현식 같으면
			...
			if(mpassword != mpasswordcheck){ // 비밀번호와 비밀번호체크와 다르면
				// equals(X) / !=(O)
	...
	// 비밀번호확인 체크
	$("#mpasswordcheck").keyup(function(){ // 비밀번호를 입력할때마다
		//	let mpassword = document.getElementById("mpassword").value; js식
		let mpassword = $("#mpassword").val(); // jquery식
		let mpasswordcheck = $("#mpasswordcheck").val();
				...
				$.ajax({
					...
					success : function(result){
						...
						if(result == 1){
							...
							$("#emailcheck").html("사용가능한 이메일입니다."); pass[5]=true;
						} // else end
					} // success end
				});
	...
	// 이메일주소 목록상자 선택시
	$("#emailselect").change(function(){ // 목록상자내 값이 변경되었을때
