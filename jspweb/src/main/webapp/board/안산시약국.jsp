...
<div class="container">
	...
	<%
	// * 공공데이터포털[web json페이지를 -> java로 변환]
		// 1. URL 가져오기[URL : java.net 패키지]
			// 1. URL 클래스 : JAVA에서 HTTP URL 객체화
		URL url = new URL("https://api.odcloud.kr/api/3035882/v1/uddi:5fae3cf5-bc15-4eba-87d8-8289b74e659b_201912202015?page=1&perPage=300&serviceKey=TBx2I5RzCKzljIjSG8GeqVSxTWTMz7q7aYTBbVYpBCsMGr7h3xgXjUkF8bqjtqnZnL0H1FdnYtX64yz%2B4v3TDg%3D%3D");	
	...
	%>
	<form action="안산시약국.jsp"> <!-- 주소 검색창 -->
		<input type="text" name="keyword" placeholder="주소 검색">
	...
	<table class="table table-hover">
		<tr>
			...
			<th>월요일 운영</th> <th>화요일 운영</th> <th>수요일 운영</th> <th>목요일 운영</th>
			<th>금요일 운영</th> <th>토요일 운영</th> <th>일요일 운영</th> <th>공휴일 운영</th>
		</tr>
	<%
		// 영업여부
		// 1. [자바에서 요일 구하기]
			// 1. Calendar : 달력 클래스[날짜(1900~9999)/시간 클래스]
		...
		for(int i = 0; i<array.length(); i++){
			JSONObject jo = array.getJSONObject(i); // i번쨰 json객체
			// 검색
			String keyword = request.getParameter("keyword");
			if(keyword != null){ // 만약에 검색이 있을경우
				int index = jo.get("주소").toString().indexOf(keyword);
				if(index == -1){continue;} // -1 인덱스는 없다는 의미
				// for, while문 : 1. break -> 반복문 종료 2. continue -> 반복문 재실행
			} // if end

		// iterator : 인덱스가 없는(set) 리스트를 순회하는 인터페이스
		/* Iterator<JSONObject> iterator = jo.keys();
		while(iterator.hasNext()){ // hasNext() : 다음 객체가 있는지 확인(iterator)
			String key = iterator.next().toString();
			if(key.equals(현재요일+"요일 운영") ){ // 현재 요일과 동일한 키만 호출
				jo.put("영업여부", jo.get(key) ); // i번째 json 객체에 새로운 엔트리[키:값] 추가
			} // if end
		} // while end */
		
		// 1. JSONObject.getNames(json 객체) : 해당 json객체내 key 호출
		String[] keys = JSONObject.getNames(jo);
		// 2. 영업여부 저장하는 변수 선언
		String 영업여부 = "[영업종료]";
		// 3. 모든 key 반복문 돌리기
		for(String key : keys){
		// 4. 만약에 해당 key가 현재 요일과 같으면서 -(영업시간이없다)가 아니면
			if(key.equals(현재요일+"요일 운영") && !jo.getString(key).equals("-") ){
				
				String[] 영업시간 = jo.getString(key).split("~"); // ~기준 : 오픈/마감시간 구분
	...
	%>
		<tr style="font-size: 8px;" onclick="map('<%=jo.get("주소")%>', '<%=jo.get("약국명") %>', '<%=영업여부 %>')">
			...
			<td><%=jo.get("월요일 운영") %></td> <td><%=jo.get("화요일 운영") %></td> <td><%=jo.get("수요일 운영") %></td> <td><%=jo.get("목요일 운영") %></td> 
			<td><%=jo.get("금요일 운영") %></td> <td><%=jo.get("토요일 운영") %></td> <td><%=jo.get("일요일 운영") %></td> <td><%=jo.get("공휴일 운영") %></td>
		</tr>
	<%	} %>
	</table>
</div>
...
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=611e36f0ae7933c9beba21f35064b00a&libraries=services,clusterer,drawing"></script>
<!-- 카카오맵 스크립트 -->
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=611e36f0ae7933c9beba21f35064b00a"></script>
