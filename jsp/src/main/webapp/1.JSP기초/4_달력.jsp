...
<body>
	...
	<% // 자바언어 작성되는 공간
		...
		if(request.getParameter("year") != null && request.getParameter("month") != null){
			...
			// 만약에 월이 1 미만이면 처리X 연도 1감소 월 12월로 설정
			if(month < 1){
				month = 12; year--;
			}
			// 만약에 월이 12 초과이면 연도 1증가 월 1월로 설정
			if(month > 12){
				month = 1; year++;
			}
			// 지원불가 달력이면 1900년 이하
		...
		// 3. 입력받은 값으로 달력 설정
		calendar.set(year, month-1, 1); 								// month-1 차감하는이유 -> 0:1월 ~ 1:12월 이기때문
		int sweek = calendar.get(Calendar.DAY_OF_WEEK); 				// 해당 월의 시작 요일 구하기(1일구하기)
	...
	<table>
		...
		<tr>
			...
			<% for(int i = 1; i<sweek; i++){%>
				<td> </td> <!-- 해당 셀 : 공백 -->
			<% } %>
			<!-- 1일부터 마지막일까지 1씩 증가하면서 출력 -->
			<% for(int i = 1; i<=eday; i++){ %>
				<td> <%=i %> </td> <!-- 일수 출력 -->
				<!-- 7배수 마다 줄바꿈 처리(토요일 마다) -->
				<% if(sweek % 7 == 0){ %>
					</tr> <tr> <!-- 테이블 행 바꿈 -->
				<% } %>
