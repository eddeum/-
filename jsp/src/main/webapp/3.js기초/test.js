	...
	js문법
		...
		3. 내장객체/메소드
			...
			2. document : 문서 객체[현 html 객체화]
				...
				2. .getElementById("html 해당id명") 	: 해당 태그 가져오기
					...
					.textContent("해당태그에 넣을 데이터")	: 해당 태그에 데이터 넣기
					.style.css속성명 = 속성값			: 해당 태그에 css 속성 적용
					.value							: 해당 태그에 value 값 가져오기[div/span X, input/select O : value 속성이 있는 태그만 가능]
		...
		4. 반복문 / 제어문 = jaca와 동일
 		...
 		5. 함수 선언
 			...
 			2. function 함수명(인수){실행문} : 인수가 있을 때
...
//메시지4 = "자바 const변수 값 변경";	alert(메시지4);
	// 변수 const : 고정값, 변수명 중복불가능, 메모리 초기화 불가능
...
function 함수1(){ // html 태그내 이벤트 태그에서 함수호출해야 실행 가능
