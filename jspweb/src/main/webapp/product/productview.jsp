...
<%@include file="../header.jsp" %>
		<%
			int pnum = Integer.parseInt(request.getParameter("pnum") ); // 제품번호 요청
			Product p = ProductDao.getproduDao().getproduct(pnum); // 제품 호출
			ArrayList<Stock> stocks = ProductDao.getproduDao().getstock(pnum); // 해당 제품의 재고
			
			Set<String> sizelist = new TreeSet<>(); // 사이즈 중복제거(set컬렉션) 선언
			for(Stock s : stocks){sizelist.add(s.getSsize() );} // 사이즈 목록 만들기
			
			Set<String> colorlist = new TreeSet<>(); // 컬러 중복제거(set컬렉션) 선언
			for(Stock s : stocks){colorlist.add(s.getScolor() );} // 사이즈 목록 만들기
			
			DecimalFormat df = new DecimalFormat("###,###원"); 					// 천단위 구분 쉼표
			Float price = p.getPprice() - (p.getPprice() * p.getPdiscount());  	// 할인금액 계산
		 	Float point = price * 0.01f; //java 기본타입 : 정수형=int / 실수형=float	// 포인트금액 계산
		%>
			...
			<div class="col-md-6"> <!-- 정보 -->
				<div class="infobox">
					 ...
					 <table class="table info_t my-5">
					 	...
					 	<tr>
					 		<td>SAVE</td> <td><%=df.format(point) %>(1%)</td>
					 	</tr>
