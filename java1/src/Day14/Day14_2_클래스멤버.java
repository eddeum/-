		...
		// 클래스 멤버
			// 1. 필드
				// 1. 접근제한자(생략시 default)
					...
					// default(생략시) : 동일한 패키지내 사용가능
					// protected : 동일한 패키지[폴더]내 사용가능(단 : 서로다른 패키지의 슈퍼클래스 접근가능)
				// * : static, final = 상수
				// 2. 자료형(클래스, 기본자료형)
					// 1. 기본자료형[int, double, char, boolean 등]
					// 2. 클래스[사용자가 만든 클래스] : 회원클래스, 게시판클래스, 제품클래스, 차클래스
					// 3. 클래스[미리 만들어진 클래스] : API(String, Scanner, ArrayList 등)
					// 4. 인터페이스
					// 5. [ ] 배열
				// 3. 변수명(객체명)
					// * 본인이 원하는대로 하되 중복이름X
					// * 클래스명의 소문자로[Scanner scanner, Date date 등]
			...
			// 3. 메소드[1. 코드 묶음  2. 재활용[재사용]  3. 인수에 따른 서로 다른 결과물  4.반환값
				...
				// 4. 인수[매개변수]
					...
					// 객체명.메소드(3)  ----> 메소드(int 변수명)
						// * 인수의 자료형 동일하되 변수명은 달라도됨
							// 객체명.메소드(유재석) ----> 메소드(int 변수명) : X
								// 문자열을 보냇으면 문자열로 받아라
									// 					  ----> 메소드(String 변수명) : O
		Day13_3_달력.달력(2022,5);
